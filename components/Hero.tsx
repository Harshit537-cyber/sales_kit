...
export default function Hero() {
  ...
  const handleScrollToNext = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    if (heroRef.current) {
      const nextElement = heroRef.current.nextElementSibling as HTMLElement | null;
      const targetTop =
        (nextElement
          ? nextElement.getBoundingClientRect().top + window.scrollY
          : heroRef.current.offsetTop + heroRef.current.offsetHeight) + 240;
  ...
  return (
    <section
      ref={heroRef}
      className="relative bg-[#170E08] text-[#FAF5EE] overflow-hidden border-b border-[#3D2817] pt-28 sm:pt-32 md:pt-36 pb-24 sm:pb-28 md:pb-32"
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center flex flex-col items-center">
        ...
        <div className="mt-7 sm:mt-8 flex items-center justify-center">
          <button
            ...
            className="cursor-pointer px-7 py-3.5 rounded-xl bg-[#C28E58] hover:bg-[#b37f4a] text-white font-semibold text-sm shadow-lg shadow-[#C28E58]/20 transition-all hover:scale-[1.02] active:scale-95"
          >
